...
The `ggalluvial` package implements an alluvial plot as an `ggplot2` [see @ggalluvial]. For example, you may use the `titanic` data to examine who survived the sinking of the Titanic. The alluvial chart helps us depict how socio-demographic variables such as sex, age, or class are related in terms of survival. The `geom_stratum()` function draws boxes that display the frequency of each group level and the `geom_alluvium()` shows the flow between groups.
