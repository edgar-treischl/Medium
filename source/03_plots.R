...
school_alluvial <- function() {
  ...
  ggplot(data = df,
    ...
    scale_fill_manual(values = c("#94d2bd", "#005f73"),
...
mosaic_plot <- function(variables) {
  ...
  ggplot(df) +
    geom_mosaic(aes(x = product(Sex), fill = Survived),
                alpha = 1,
                color = c("#005f73", "black", "#005f73", "black"))+
    theme_minimal(base_size = 12, base_family ="Lato")+
    ylab("Survived")+
    xlab("Sex")+
    theme(legend.position = "bottom")+
...
ridge_plot <- function() {
  ...
  ggplot(df, aes(x = lifeExp, y = forcats::fct_rev(continent), fill = after_stat(x))) +
...
dot_plot <- function() {
  ...
  m2 <- lm(body_mass_g ~ bill_length_mm + flipper_length_mm + sex,
           data = penguins
  )
  ...
  dwplot(list(m1, m2),
    ...
    scale_color_manual(values=c("#005f73", "#94d2bd"))
